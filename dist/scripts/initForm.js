...
async function initializeForm() {
    ...
    try {
        ...
        for (const topic of topics) {
            await form_services_1.FormService.createTopic(admin.id, topic.name, topic.description);
            console.log(`✅ Tópico criado: ${topic.name}`);
        }
        // Buscar tópicos criados
        const createdTopics = await form_services_1.FormService.getTopics(admin.id, 'ADMIN');
        // Criar perguntas para cada tópico
        console.log('\n📝 Criando perguntas...');
        for (const topic of createdTopics) {
            const questions = questionsByTopic[topic.name] || [];
            for (const questionText of questions) {
                await form_services_1.FormService.createQuestion(topic.id, questionText, 'Descrição da pergunta...', ['BAIXA', 'MEDIA', 'ALTA'][Math.floor(Math.random() * 3)]);
